...
Os conjuntos de dados apresentados a seguir são provenientes de um experimento dose-resposta, conduzido para avaliar a influência dos extratos vegetais "aquoso frio de folhas", "aquoso frio de frutos" e um extrato químico, respectivamente, na morte de um determinado tipo de caramujo. Para cada conjunto, ajuste um modelo logístico linear simples e um modelo log-log linear simples. Para o melhor ajuste, usando envelope como critério, encontre um intervalo assintótico de 95% para a dose letal $DL_{50}$, contrua as bandas de confiança e verifique se há indícios de sobredispersão aplicando um teste apropriado.
